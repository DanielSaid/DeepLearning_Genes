...
Don't pay too much attention to the printed text, as the random selection process will continually print what it currently has (can't easily turn this off).
...
def main():
    ...
    # Dan: I added the following lines ------------------------------ -----------------------------
    
    nested = True  # Dan: if true, input file needs to be selected below (scroll down)
    orthologs = False  # Dan: change here as desired
    
    # Dan: names of output files need to be manually adjusted (as desired)
    # Dan: if you want to create the 'core' of a nest (e.g. 20), nested must be set to False!

    if nested == False:
        for k in range(80,81):  # Dan: desired number of genes
            numb_genes = k
            for j in range(99,100):  # Dan: desired number (i.e. names) of sets
                x_satisfied = False  # Dan: set to True if you want to select only one domain (not recommended)
                y_satisfied = False
                file1 = "data_X%d"%(numb_genes)+"_%d"%(j) + ".p"
                file2 = "data_%d"%(numb_genes)+"_%d"%(j) + "_human.p"  # Dan: change to desired domain here!
                ...
                while not x_satisfied or not y_satisfied:
                    ...
                    if not x_satisfied:
                        ...
                        elif orthologs:  # Dan: occurs only if we select unnested orthologs (e.g. 20)
                            continue
        ...
        numb_genes = 15  # Dan: number of genes to add
        numb_genes_core = 35  # Dan: size of set to build upon
        numb_genes_out = numb_genes + numb_genes_core
        for j in range(1,2):  # Dan: desired number (and names) of output sets
            y_satisfied = False  # Dan: set True if only one domain is desired (not recommended)
            x_satisfied = False
            ...
            "/data_%d"%(numb_genes_core) + "_%d"%(j) + "_human_nest.p"  # Dan: change name here as desired
