...
import CompoundLists  # Dan: added to test read_data
...
def read_data(compounds, x_type="rat_vitro", y_type="human_vitro", gene_list=None, dataset="big",numb_genes=50,orthologs=False):
    if x_type == y_type:
        ...
        if gene_list == 'random':
            ...
            if orthologs == False:
                ...
                try:
                    ...
                    data = np.delete(data,0,0)
                    genes_first = gene_list
                    genes_second = []  # Dan: choosing the correspoding human names
                    tempppppppppp = data[:,1].tolist()
                    for i in genes_first:
                        temp = tempppppppppp.index(i)
                        genes_second.append(data[temp,2])
                except ValueError:
                    print("Oops! There could be no equivalent to gene ", i, " in the human data be found. Are you sure you provided orthologs?")
        print("X genes:", genes_first)
        print("Y genes:", genes_second)
        
        print("You are using ", len(compounds), " compounds")
...
def main():
    ...
    """
    compound_list = CompoundLists.GENERAL_47
    a,b,c,d,e = read_data(compound_list,gene_list='orthologs')  # Dan: lines added for testing
