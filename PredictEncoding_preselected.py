...
def merge_autoencoders(inputs, encoder, decoder):
    ...
    # print(network.summary()) Dan: I commented this out

    adam = Adam(lr=0.0005, decay=0.00001)
    network.compile(loss='mae', optimizer=adam, metrics=['mae', 'mse'])
    # network.compile(loss=custom_loss, optimizer=adam, metrics=['mae', 'mse'])
    return network


def autoencoder_model(inputs): # Dan: this is the 'general' autoencoder model
    # Encoder

    input_layer = Input(shape=(inputs,))
    encoded = Dense(70, activation='relu')(input_layer) # Dan: why (not) 70?
...
def train_autoencoders_alex(X_train, X_valid, Y_train, Y_valid, norms_X, norms_Y):  # Dan: I changed the name
...
def train_autoencoders(X_train, X_valid, Y_train, Y_valid, norms_X, norms_Y):
    # Create and train autoencoder for first domain
    print("Training autoencoder on first domain")
    # d1_Y_train = X_to_doubled(X_train)
    # d1_Y_valid = X_to_doubled(X_valid)
    ...
    # d2_X_train = Y_to_doubled(Y_train)
    # d2_X_valid = Y_to_doubled(Y_valid)
...
# Train a random forest, elastic net or kNN and output predictions for the test data
...
def random_split_evaluation(X, Y, compounds, model='autoencoder', iterations=50):
    ...
    for i in range(iterations):
        ...
        if model == 'autoencoder':
            X_train, X_valid, Y_train, Y_valid, recon_train, recon_valid, errors = train_autoencoders(X_train, X_valid,
                                                                                                      Y_train, Y_valid,
                                                                                                      norms_X, norms_Y)
        else:
...
def leave_one_out_evaluation(X, Y, compounds, model='autoencoder', x_vivo_arg=False, y_vivo_arg=False):
    ...
    # A bit hacky, but necessary for this to be called from another file
    ...
    for i, exclude_compound in enumerate(unique_compounds):
        ...
        if model == 'autoencoder':
            ...
            X_train, X_valid, Y_train, Y_valid, recon_train, recon_valid, errors = cnn_single_convo_data_preprocessing(
                X_train, X_valid,
                Y_train, Y_valid,
                norms_X, norms_Y)

        elif model == 'untrained_ae':
            X_train, X_valid, Y_train, Y_valid, recon_train, recon_valid, errors = train_autoencoders_alex(
                X_train, X_valid,
                Y_train, Y_valid,
                norms_X, norms_Y)
        else:
...
def main():
    ...
    gene_list = GeneLists.NAFLD  # Dan: select desired gene list here (needs to be provided in GeneLists.py)!
    ...
    rf -> random forest
    autoencoder -> pre-trained AE
    untrained_ae -> un-trained AE
    cnn -> convolutional neural network
    """

    data = leave_one_out_evaluation(X, Y, data_compounds,model='untrained_ae',x_vivo_arg=x_vivo, y_vivo_arg=y_vivo)

    """ Dan: the following can be used to save the actual prediction values
    out_file_name = "vivo_nafld_R3.mat"  # Dan: this outputs the actual prediction values
